...
set VH_TRAIN $DATA_DIR/TRAIN/Video-Holmes/train.parquet
set VH_TEST $DATA_DIR/BENCHMARK/Video-Holmes/test.parquet
set LVR_3K $DATA_DIR/TRAIN/LongVideoReason/train_mcq_3k.parquet
set LVR_10K $DATA_DIR/TRAIN/LongVideoReason/train_mcq_10k.parquet
set LVR_20K $DATA_DIR/TRAIN/LongVideoReason/train_mcq_20k.parquet
set LVR_FULL $DATA_DIR/TRAIN/LongVideoReason/train.parquet
set LVR_TEST $DATA_DIR/BENCHMARK/LongVideoReason/test.parquet
...
EXP=framethinker_coldstart_vh+lvr20k_TIS_thr2_isbn \
ray_job_submit --no-wait --skip-exists --runtime-env ../runtime_env.yaml \
    --submission-id $EXP -- bash -c \
    """EXP=$EXP bash examples/framethinker/train_frame_thinker.sh \
    data.train_files=[$VH_TRAIN,$LVR_20K] \
    data.val_files=[$VH_TEST,$LVR_TEST] \
    algorithm.rollout_correction.rollout_is=token \
    algorithm.rollout_correction.rollout_is_threshold=2.0 \
    algorithm.rollout_correction.rollout_is_batch_normalize=true \
...
EXP=framethinker_coldstart_vh+lvrfull_TIS_thr2_isbn \
ray_job_submit --no-wait --skip-exists --runtime-env ../runtime_env.yaml \
    --submission-id $EXP -- bash -c \
    """EXP=$EXP bash examples/framethinker/train_frame_thinker.sh \
    data.train_files=[$VH_TRAIN,$LVR_FULL] \
    data.val_files=[$VH_TEST,$LVR_TEST] \
    algorithm.rollout_correction.rollout_is=token \
    algorithm.rollout_correction.rollout_is_threshold=2.0 \
    algorithm.rollout_correction.rollout_is_batch_normalize=true \
