...
MODEL_PATH=$MODEL_WEIGHT_DIR/qwen2_5vl_7b_full_framethinker_sft
...
SAVE_CHECKPOINT_DIR=${SAVE_CHECKPOINT_DIR:-$PROJECT_DIR/checkpoints/}

if [ ! -f "$TOOL_CONFIG_PATH" ]; then
    echo "Missing tool config: $TOOL_CONFIG_PATH"
    echo "Make sure recipe/framethinker/config/video_think_tool_config.yaml exists or set TOOL_CONFIG_PATH"
    exit 1
fi
