...
class RayPPOTrainer:
    ...
    def _validate(self):
        # Temporarily disable vLLM sleep/wake (free_cache_engine) during validation
        free_cache_backup = self.config.actor_rollout_ref.rollout.free_cache_engine
        with open_dict(self.config.actor_rollout_ref.rollout):
            self.config.actor_rollout_ref.rollout.free_cache_engine = False
        if hasattr(self, "actor_rollout_wg"):
            try:
                self.actor_rollout_wg.set_free_cache_engine(False)
            except Exception as e:
                print(f"[validate] disable free_cache_engine on workers failed: {e}")
        ...
        try:
            ...
            # Restore original free_cache_engine setting
            with open_dict(self.config.actor_rollout_ref.rollout):
                self.config.actor_rollout_ref.rollout.free_cache_engine = free_cache_backup
            if hasattr(self, "actor_rollout_wg"):
                try:
                    self.actor_rollout_wg.set_free_cache_engine(free_cache_backup)
                except Exception as e:
                    print(f"[validate] restore free_cache_engine on workers failed: {e}")
